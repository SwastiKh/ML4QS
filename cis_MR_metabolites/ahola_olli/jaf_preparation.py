...
OUTPUT_PATH =os.path.join(HOME,'cis_MR_metabolites', 'ahola_olli', 'results')
...
###############################################################################
generic_jaf = pd.read_csv( os.path.join(ROOT_DIR,'ahola_olli__mapper.tsv'),
            sep='\t', index_col=0)
...
# exposure index
# exposure index
generic_jaf['exposure_path'] = os.path.join(HOME, 'cis_MR_metabolites', 'ahola_olli', 'ahola_olli__mapper.tsv')
...
###############################################################################
jaf.to_csv(os.path.join(HOME,'cis_MR_metabolites', 'ahola_olli', 'jaf.txt'), sep='\t', header=True, index=False)
