...
OUTPUT_PATH =os.path.join(HOME,'cis_MR_metabolites', 'interval', 'results')
...
###############################################################################
generic_jaf = pd.read_csv( os.path.join(ROOT_DIR,'interval_pqtl_mapper.tsv'),
            sep='\t', index_col=0)
...
# exposure index
# exposure index
generic_jaf['exposure_path'] = os.path.join(HOME, 'cis_MR_metabolites', 'interval', 'interval_pqtl_mapper.tsv')
...
###############################################################################
jaf.to_csv(os.path.join(HOME,'cis_MR_metabolites', 'interval', 'jaf.txt'), sep='\t', header=True, index=False)
