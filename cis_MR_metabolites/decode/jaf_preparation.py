...
OUTPUT_PATH =os.path.join(HOME,'cis_MR_metabolites', 'decode', 'results')
...
# exposure index
# exposure index
generic_jaf['exposure_path'] = os.path.join(HOME, 'cis_MR_metabolites', 'decode', 'decode_pqtl_mapper.tsv')
...
###############################################################################
jaf.to_csv(os.path.join(HOME,'cis_MR_metabolites', 'decode', 'jaf.txt'), sep='\t', header=True, index=False)
