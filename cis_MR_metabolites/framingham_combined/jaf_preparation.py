...
OUTPUT_PATH =os.path.join(HOME,'cis_MR_metabolites', 'framingham_combined', 'results')
...
###############################################################################
generic_jaf = pd.read_csv( os.path.join(ROOT_DIR,'framingham_mapper.tsv'),
            sep='\t', index_col=0)
...
# exposure index
# exposure index
generic_jaf['exposure_path'] = os.path.join(HOME, 'cis_MR_metabolites', 'framingham_combined', 'framingham_mapper.tsv')
...
###############################################################################
jaf.to_csv(os.path.join(HOME,'cis_MR_metabolites', 'framingham_combined', 'jaf.txt'), sep='\t', header=True, index=False)
