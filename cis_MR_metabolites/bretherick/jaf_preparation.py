...
OUTPUT_PATH =os.path.join(HOME,'cis_MR_metabolites', 'bretherick', 'results')
...
###############################################################################
generic_jaf = pd.read_csv( os.path.join(ROOT_DIR,'bretherick_pqtl_mapper.tsv'),
            sep='\t', index_col=0)
...
# exposure index
# exposure index
generic_jaf['exposure_path'] = os.path.join(HOME, 'cis_MR_metabolites', 'bretherick', 'bretherick_pqtl_mapper.tsv')
...
###############################################################################
jaf.to_csv(os.path.join(HOME,'cis_MR_metabolites', 'bretherick', 'jaf.txt'), sep='\t', header=True, index=False)
