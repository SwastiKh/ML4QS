...
# -- ------------------- Lightgbm -------------------
pbounds = {"max_depth" : (1, 20), "eta" : (0.01, 0.5), "gamma": (0.01, 0.5), "subsample" : (0.5, 1.0), "colsample_bytree" : (0.5, 1.0)}
def train_lightgbm_wrapper(max_depth, eta, gamma, subsample, colsample_bytree):
    # num_boost_round = int(round(num_boost_round))
    max_depth = int(round(max_depth))
    accuracy = train_lightgbm_model(X_train, y_train, X_test, y_test, max_depth, eta, gamma, subsample, colsample_bytree)
    return accuracy
# create instance of optimizer
optimizer3 = BayesianOptimization(
    f = train_lightgbm_wrapper,
    pbounds = pbounds,
    random_state = 1
)
...
# create logger 
logger = JSONLogger(path = "./tunning_lightgbm.log")
