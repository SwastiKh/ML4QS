...
OUTPUT_PATH =os.path.join(HOME,'cis_MR_metabolites', 'gilly', 'results')
...
###############################################################################
generic_jaf = pd.read_csv( os.path.join(ROOT_DIR,'gilly_pqtl_mapper.tsv'),
            sep='\t', index_col=0)
...
# exposure index
# exposure index
generic_jaf['exposure_path'] = os.path.join(HOME, 'cis_MR_metabolites', 'gilly', 'gilly_pqtl_mapper.tsv')
...
###############################################################################
jaf.to_csv(os.path.join(HOME,'cis_MR_metabolites', 'gilly', 'jaf.txt'), sep='\t', header=True, index=False)
